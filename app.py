...
with col2:
    birth_pref = st.selectbox("📍 出生都道府県", options=list(PREFECTURE_DATA.keys()), index=12)
...
for i, event in enumerate(st.session_state.events):
    with st.container(border=True):
        ...
        # ▼▼▼ 変更点2 ▼▼▼: min_valueとmax_valueを追加
        event['date'] = col2.date_input("発生年月",
                                      min_value=datetime(1900, 1, 1),
                                      max_value=datetime(2099, 12, 31),
                                      value=event.get('date', datetime(2010,1,1)),
                                      key=f"date_{i}")
...
if st.button("出生時刻を計算する 🚀", type="primary", disabled=(not st.session_state.events)):
    ...
    for i, minute_of_day in enumerate(range(0, 24 * 60, 4)):
        ...
        natal_chart["birth_date"] = birth_dt
        score = 0
        evidence = []
